...
/* ---------------- Kanboard Colors (per user) --------------- */
// Speichert die individuell gewählten Farben pro Mitarbeiter (nur im Browser / localStorage).
const KANBOARD_USER_COLORS_KEY = "stenau_kanboard_user_colors_v1";

function safeLoadKanboardUserColors() {
  if (typeof window === "undefined") return {};
  try {
    const raw = window.localStorage.getItem(KANBOARD_USER_COLORS_KEY);
    if (!raw) return {};
...
function safeSaveKanboardUserColors(map) {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.setItem(KANBOARD_USER_COLORS_KEY, JSON.stringify(map || {}));
  } catch {
    // ignore
  }
}

// Deterministische Default-Farbe, falls der User noch nichts gewählt hat
function defaultUserColor(seed) {
  const palette = [
    "#3b82f6", "#22c55e", "#a855f7", "#f97316", "#06b6d4",
    "#ef4444", "#84cc16", "#eab308", "#14b8a6", "#f43f5e",
  ];
  const s = String(seed || "");
  let h = 0;
...
function KanboardPanel({ isAdmin = false }) {
  ...
  useEffect(() => {
    setUserColors(safeLoadKanboardUserColors());
  }, []);

  const setUserColor = (userId, color) => {
    setUserColors((prev) => {
      const next = { ...(prev || {}), [userId]: color };
      safeSaveKanboardUserColors(next);
      return next;
    });
  };
      ...
      <div style={styles.kanbanGrid}>
        ...
        <div style={{ display: "grid", gap: 12, gridTemplateColumns: "repeat(auto-fit, minmax(420px, 1fr))", alignItems: "start" }}>
          {(filterUserId === "all" ? (members || []) : (members || []).filter((m) => String(m.id) === String(filterUserId))).map((m) => {
            ...
            const colColor = (userColors && userColors[m.id]) || defaultUserColor(m.id);
            const by = {
